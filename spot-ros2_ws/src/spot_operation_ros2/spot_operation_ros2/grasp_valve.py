...
from geometry_msgs.msg import (
    Pose,
    PoseStamped,
    TransformStamped,  # <-- ADD THIS LINE
)
...
class ValveGraspNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter(
            "fjt_action_server", "/arm_controller/follow_joint_trajectory"
        )
        fjt_action_name = (
            self.get_parameter("fjt_action_server").get_parameter_value().string_value
        )
        ...
        self.get_logger().info("Connected to MoveGroup action server successfully!")
        ...
        self.get_logger().info("Creating Cartesian Path service client...")
        ...
        self.get_logger().info("Waiting for /compute_cartesian_path server...")
        if not self.cartesian_path_client.wait_for_service(timeout_sec=10.0):
            ...
            raise RuntimeError("Cartesian Path service not available")

        self.get_logger().info(f"Waiting for {fjt_action_name} server...")
        ...
        self.get_logger().info("All clients and services are connected!")
        ...
        self.gripper_cmd_pub = self.create_publisher(Float64, "gripper/command", 10)
        ...
        self.get_logger().info(
            "  - /plan_grasp: Plan motion only (no execution, no gripper)"
        )
        self.get_logger().info(
            "  - /execute_grasp: Execute planned motion with pre-grasp"
        )
        self.get_logger().info(
            "  - /grasp_valve: Complete grasp (plan + execute + gripper)"
        )
        ...
        self.close_valve_service = self.create_service(
            Trigger,
            "close_valve",
            self.close_valve_cartesian_callback,  # <-- Point to new callback
            callback_group=self.callback_group,
        )
        ...
        self.get_logger().info("Available services:")
        self.get_logger().info(
            "  - /close_valve: Close valve using Cartesian path (after grasp)"
        )
    ...
    def _extract_gripper_angles(self):
        ...
        try:
            ...
            self.get_logger().info(
                f"  Open (pre-grasp): {self.gripper_open_angle:.4f} rad"
            )
            self.get_logger().info(
                f"  Close (grasp):    {self.gripper_close_angle:.4f} rad"
            )
            self.get_logger().info("=" * 60)
    ...
    def valve_pose_callback(self, msg: Detection3DArray):
        ...
        if len(self.valve_poses) == 1:
            self.get_logger().info(
                f"First valve detection received in frame: {detection.header.frame_id}"
            )
    ...
    def construct_goal_constraints(
        self,
        pose_stamped: PoseStamped,
        tolerance_pos: float = None,
        tolerance_angle: float = None,
    ) -> Constraints:
        """
    ...
    def compute_grasp_pose_in_planning_frame(self):
        ...
        #         f"Insufficient detections for averaging: {len(self.valve_poses)}/3"
        #     )
        #     return None
        ...
        try:
            transform = self.lookup_transform(
                self.PLANNING_FRAME, "lever_pivot", timeout=5.0
            )
            self.get_logger().error("No transform from lever pivot!")
            ...
            self.get_logger().info(
                f"Computed grasp pose in {self.PLANNING_FRAME} frame"
            )
            self.get_logger().info(
                f"Position: [{grasp_pose.pose.position.x:.3f}, "
    ...
    def open_gripper(self):
        ...
        self.get_logger().info(
            f"Opening gripper to {self.gripper_open_angle:.4f} rad..."
        )
        cmd_msg = Float64()
    ...
    def close_gripper(self):
        ...
        self.get_logger().info(
            f"Closing gripper to {self.gripper_close_angle:.4f} rad..."
        )
        cmd_msg = Float64()
    ...
    def plan_grasp_callback(self, request, response):
        ...
        try:
            ...
            if grasp_pose is None:
                ...
                response.message = (
                    "Failed to compute grasp pose. Check valve detection and TF tree."
                )
            ...
            if error_code != result.result.error_code.SUCCESS:
                response.success = False
                response.message = (
                    f"Motion planning failed with error code: {error_code}"
                )
                self.get_logger().error(response.message)
                return response
            ...
            response.message = (
                "Motion planned successfully! Use /execute_grasp to execute."
            )
            self.get_logger().info("=" * 50)
    ...
    def execute_grasp_callback(self, request, response):
        ...
        try:
            ...
            if error_code != result.result.error_code.SUCCESS:
                response.success = False
                response.message = (
                    f"Motion execution failed with error code: {error_code}"
                )
                self.get_logger().error(response.message)
                return response
    ...
    def grasp_service_callback(self, request, response):
        ...
        try:
            ...
            self.get_logger().info(
                "Step 1: Computing grasp pose from valve detection..."
            )
            grasp_pose = self.compute_grasp_pose_in_planning_frame()
            ...
            self.get_logger().info(
                "Step 2: Planning and executing motion to grasp pose..."
            )
            ...
            if error_code != result.result.error_code.SUCCESS:
                response.success = False
                response.message = (
                    f"Motion planning/execution failed with error code: {error_code}"
                )
                self.get_logger().error(response.message)
                return response
    ...
    def _generate_rotation_waypoints(
        ...
        Args:
            start_pose_stamped: The starting PoseStamped (e.g., grasp pose) in the PLANNING_FRAME.
            pivot_frame: The TF frame to rotate around (e.g., "lever_pivot").
            rotation_axis: The axis of rotation ('x', 'y', or 'z') in the pivot_frame.
            total_angle_rad: The total angle to rotate (negative for clockwise/close).
            num_steps: The number of waypoints to generate.

        Returns:
            A list of Pose messages in the PLANNING_FRAME, or None on failure.
        """
        self.get_logger().info(
        ...
        try:
            # 1. Get the transforms between the planning frame and the pivot frame
            ...
            # 2. Transform the *starting grasp pose* from the body frame into the *pivot frame*
            self.get_logger().error("do_transform_pose.")
            start_pose_in_pivot_msg = do_transform_pose(
                start_pose_stamped.pose, transform_body_to_pivot
            )
            self.get_logger().error("do_transform_pose done.")

            # 3. Generate waypoints by applying incremental rotations IN THE PIVOT FRAME
            for i in range(1, num_steps + 1):
                angle = (total_angle_rad / num_steps) * i

                # Create a pure rotation quaternion for this step
                if rotation_axis == "x":
                ...
                # This transform applies the rotation to the start_pose_in_pivot
                rot_transform_stamped_msg = TransformStamped()

                # We only need to populate the .transform field
                ...
                # Apply the rotation to the *original* grasp pose (in pivot frame)
                # This calculates: new_pose_in_pivot = T_rotation * start_pose_in_pivot
                # We use do_transform_pose for the quaternion math
                self.get_logger().error("waypoint pivot do_transform_pose.")
                waypoint_in_pivot_msg = do_transform_pose(
                    start_pose_in_pivot_msg, rot_transform_stamped_msg
                )

                # 4. Transform the new waypoint *back* to the planning (body) frame
                self.get_logger().error("waypoint body do_transform_pose.")
    ...
    def close_valve_cartesian_callback(self, request, response):
        ...
        Assumes:
        1. Robot is currently at the grasp pose (from /grasp_valve).
        2. Gripper is closed.
        """
        self.get_logger().info("=" * 50)
        ...
        try:
            # Check if we have successfully grasped
            ...
            self.get_logger().info("Step 1: Generating rotation waypoints...")
            # Customize as needed: axis, angle, and steps
            ...
            # --- Step 2: Request Cartesian Path from MoveIt ---
            self.get_logger().info("Step 2: Calling /compute_cartesian_path service...")
            ...
            cartesian_req.start_state.is_diff = True  # Start from current state
            ...
            cartesian_req.max_step = 0.10  # 1cm resolution for collision checking
            cartesian_req.avoid_collisions = False

            future = self.cartesian_path_client.call_async(cartesian_req)
            ...
            if cartesian_result.fraction < 0.40:  # Check if path is complete
                response.success = False
            ...
            self.get_logger().info(
                f"Cartesian path computed successfully (fraction: {cartesian_result.fraction:.2f})."
            )

            # --- Step 3: Execute the Trajectory ---
            self.get_logger().info("Step 3: Executing trajectory...")

            # Extract the joint trajectory from the MoveIt solution
            ...
            if goal_handle is None or not goal_handle.accepted:
                response.success = False
                response.message = (
                    "Trajectory execution goal was rejected or timed out."
                )
                self.get_logger().error(response.message)
                return response
            ...
            result_future = goal_handle.get_result_async()
            fjt_result = self.wait_for_future(
                result_future, timeout_sec=45.0
            )  # Give it time to run
